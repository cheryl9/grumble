export default function ProfileDashboard({ stats, onViewAll }) {
  const cards = [
    { label: 'Friends', value: stats?.friends ?? 0, key: 'friends', icon: '🔔' },
        ...
        {cards.map((card) => (
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: '14px', fontWeight: '500', color: '#333' }}>{card.label}</span>
              {card.icon && <span style={{ fontSize: '16px' }}>{card.icon}</span>}
            </div>

            {/* Count */}
            <span style={{ fontSize: '36px', fontWeight: '700', color: '#111', lineHeight: 1.1 }}>
              {card.value}
            </span>
