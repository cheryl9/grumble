...
export default function EditProfileModal({ user, onClose, onSave }) {
  ...
  const handleSaveInfo = async () => {
    ...
    setLoading(true);
    try {
      const response = await api.put('/auth/profile', { username, phone_number });
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h2 style={{ fontSize: '18px', fontWeight: '700', marginBottom: '20px', color: '#111' }}>
          Edit Profile
        </h2>
        ...
        {error && (
          <div style={{ backgroundColor: '#fee2e2', color: '#b91c1c', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', marginBottom: '14px' }}>
            {error}
          </div>
        )}
        {success && (
          <div style={{ backgroundColor: '#dcfce7', color: '#166534', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', marginBottom: '14px' }}>
            {success}
          </div>
        )}
        ...
        {tab === 'info' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            <Field label="Phone Number" value={phone_number} onChange={setPhoneNumber} type="tel" />
            <button
