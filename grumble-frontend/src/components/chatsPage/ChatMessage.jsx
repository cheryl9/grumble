...
const ReplyPreview = ({ replyTo }) => {
  ...
  return (
    <div className="text-xs text-gray-500 bg-white/70 border border-gray-200 rounded-xl px-2 py-1 mb-1">
      <span className="font-semibold">{replyTo.sender?.username || 'Someone'}:</span>{' '}
    ...
    return (
      <div className="flex justify-center">
        <div className="chat-poll text-center">
          <p className="text-xs font-bold text-gray-500 mb-3">{senderName} started a spin</p>
          <SpinWheel
    ...
    return (
      <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
        <div className="max-w-[75%]">
          {!isMe && <p className="text-xs text-gray-400 mb-1 ml-1">{senderName}</p>}
    ...
    <div className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
      <div className={containerClass}>
        ...
        <div>
          ...
          <div className={`px-3 py-2 rounded-2xl text-sm ${bubbleClass}`}>{text}</div>
